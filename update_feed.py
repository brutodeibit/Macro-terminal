...
BLS={'Nóminas no agrícolas (NFP)':('CES0000000001','EMPLEO',+1,3,'K'),'Tasa de desempleo':('LNS14000000','EMPLEO',-1,3,'%'),'Salarios (AHE)':('CES0500000003','EMPLEO',+1,2,','JOLTS Vacantes':('JTS000000000000000JOL','EMPLEO',+1,2,'K'),'JOLTS Tasa de abandonos':('JTS000000000000000QUR','EMPLEO',+1,2,'%')}
...
def update_us(feed):
 ...
 try:
  ...
  for name,(sid,cat,sign,imp,unit) in BLS.items():
   ...
   estnum=num(row.get('est'))
   if estnum is not None: row['surprise']=f"{v-estnum:+.2f}"
   if name not in by:inds.append(row)
 ...
 for eid,cfg in ECONOMIES.items():
  try:
   ...
   for ev in data if isinstance(data,list) else []:
    ...
    raw_actual=num(actual); raw_forecast=num(forecast)
    if any(k in lname for k in ('cpi','core cpi')) and raw_actual is not None and abs(raw_actual)>20 and (raw_forecast is None or abs(raw_forecast)<20):
     continue  # Ignora índices CPI (p. ej. 334.980) cuando este evento espera una tasa mensual/anual.
    ...
    if forecast not in (None,'') and raw_actual is not None and raw_forecast is not None:row['surprise']=f"{raw_actual-raw_forecast:+.2f}"
...
),'JOLTS Vacantes':('JTS000000000000000JOL','EMPLEO',+1,2,'K'),'JOLTS Tasa de abandonos':('JTS000000000000000QUR','EMPLEO',+1,2,'%')}
FRED={'PCE subyacente (Anual)':('PCEPILFE','INFLACIÓN',+1,3,'%'),'PCE (Anual)':('PCEPI','INFLACIÓN',+1,3,'%'),'PIB (Trimestral)':('A191RL1Q225SBEA','CRECIMIENTO',+1,3,'%'),'Initial Jobless Claims':('ICNSA','EMPLEO',-1,2,'K'),'Continuing Claims':('CCNSA','EMPLEO',-1,2,'K'),'HY OAS':('BAMLH0A0HYM2','CONDICIONES FINANCIERAS',-1,2,'%')}
